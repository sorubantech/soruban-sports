...
const students = [
  ...
  { id: 3, name: 'Karthik M', stage: 3, role: 'Bowler (Fast)', score: 55, attendance: 70, feePaid: false, batch: 'Evening U-14', joined: 'Feb 2025', avatar: 'KM', feeAmount: 3500, feeDays: 12, promotionReady: false },
  ...
  { id: 5, name: 'Deepak R', stage: 4, role: 'Batsman', score: 74, attendance: 60, feePaid: false, batch: 'Weekend All Ages', joined: 'Jun 2025', avatar: 'DR', feeAmount: 4000, feeDays: 22, promotionReady: false },
  ...
  { id: 9, name: 'Vikram T', stage: 2, role: 'Batsman', score: 58, attendance: 65, feePaid: false, batch: 'Evening U-14', joined: 'Jul 2025', avatar: 'VT', feeAmount: 3500, feeDays: 8, promotionReady: false },
...
screens['create-post'] = () => `
  <div class="screen-pad screen-enter">
    ...
    <div style="display:flex;gap:8px;margin-bottom:16px;">
      ...
      <button style="flex:1;padding:10px;border-radius:10px;border:1px solid var(--border);background:var(--bg-card);display:flex;align-items:center;justify-content:center;gap:6px;font-size:13px;color:var(--text-secondary);cursor:pointer;" onclick="showToast('Announcement mode', 'info')">
        <i class="fas fa-bullhorn" style="color:var(--info);"></i> Alert
      </button>
    </div>
...
screens['more'] = () => `
  <div class="screen-pad screen-enter">
    ...
    <!-- Academy Management -->
    <div class="section-title" style="margin-bottom:10px;margin-top:4px;">Academy Management</div>
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:12px;overflow:hidden;margin-bottom:16px;">
      ...
      <div class="chat-item" onclick="showToast('Contact support: support@soruban.com', 'info')">
        <div style="width:36px;height:36px;border-radius:10px;background:var(--bg-tertiary);display:flex;align-items:center;justify-content:center;"><i class="fas fa-envelope" style="color:var(--text-secondary);font-size:14px;"></i></div>
        <div class="chat-info"><div class="chat-name" style="font-size:14px;">Contact Us</div></div>
        <i class="fas fa-chevron-right" style="color:var(--text-tertiary);font-size:12px;"></i>
      </div>
  ...
  return `
    <div class="screen-pad screen-enter">
      ...
      <button class="btn btn-outline btn-full" style="margin-bottom:16px;font-size:13px;color:var(--danger);border-color:var(--danger);" onclick="showToast('Reminders sent to ${pendingStudents.length} students!', 'success')">
        <i class="fas fa-bell"></i> Send Reminder to All (${pendingStudents.length})
      </button>

      <!-- Pending List -->
      <div class="section-title" style="margin-bottom:10px;">Pending Fees</div>
      ${pendingStudents.map((s, i) => `
        <div class="fee-item fade-in-up fade-in-up-${Math.min(i + 2, 5)}">
          <div class="fee-avatar">${s.avatar}</div>
          <div class="fee-info">
            <div class="fee-name">${s.name}</div>
            <div class="fee-detail">${s.batch} · ${s.feeDays || 10} days overdue</div>
          </div>
          <div style="text-align:right;">
            <div class="fee-amount" style="color:var(--danger);">₹${(s.feeAmount || 3500).toLocaleString()}</div>
            <div style="display:flex;gap:4px;margin-top:6px;">
              <button style="padding:4px 8px;font-size:10px;border-radius:6px;border:1px solid var(--success);background:var(--success-bg);color:var(--success);cursor:pointer;font-weight:600;" onclick="showToast('Payment recorded for ${s.name}', 'success')">Record</button>
              <button style="padding:4px 8px;font-size:10px;border-radius:6px;border:1px solid var(--warning);background:var(--warning-bg);color:var(--warning);cursor:pointer;font-weight:600;" onclick="showToast('Reminder sent to ${s.name}', 'success')">Remind</button>
            </div>
          </div>
        </div>
      `).join('')}
...
document.addEventListener('DOMContentLoaded', () => {
  initNav({
    ...
    tabMapping: {
      ...
      'feed': ['create-post'],
      'more': ['curriculum', 'fee-management', 'staff-chat', 'admissions', 'staff-management', 'settings', 'chat-detail']
    }
  });
});
